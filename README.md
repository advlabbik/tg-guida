...
- [ ] **Allargare il token GitHub alla repo tedesca.** Lo smistamento per evento è in produzione e provato su entrambi i rami (31/8), ma il PAT vede solo `tg-guida`: sul ramo Germania/Austria GitHub risponde `The resource you are requesting could not be found` e in Slack compare «⚠️ NON CREATA». Si aggiusta **modificando il token esistente**, non facendone un altro: <https://github.com/settings/personal-access-tokens> → il token usato da n8n → *Repository access* → aggiungere `evento-tge-ger-austria-2026`. La label `feedback` in quella repo è già creata.
