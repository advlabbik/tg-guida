...
| `qr.js` | **Il QR personale del ritiro pacco — acceso** (`config.js` → `EVENTO_CONFIG.qr`). Legge `?qr=` dall'indirizzo, controlla la forma del codice, decide cosa salvare e cosa mostrare. Arriva dal template (`motore/qr.js`) e **va tenuto identico byte per byte**, come `uso.js`. Il piano e le prove: sezione «Il QR del ritiro pacco dentro l'app» nel README della [PR #30](https://github.com/advlabbik/tg-guida/pull/30) |
| `lean-qr.js` | La libreria che disegna il QR (lean-qr 2.7.4, MIT), dentro l'app perché funzioni senza segnale. Copia identica di `motore/lean-qr.js` del template |
...
- **Info**: schede a tema (gli orari, prima di partire, arrivare a Rovereto, sul percorso, durante l'evento, regole e vantaggi) con ricerca full-text. **La prima scheda, sopra i gruppi a tema, è «Il tuo QR per il ritiro pacco»** (dal 17/9/2026, ramo `qr-nell-app`): il QR personale arriva con il link `?qr=<codice>` dell'email del ritiro pacco, si salva sul telefono e si ridisegna dal codice anche senza segnale; chi ha già l'app installata in Home (su iPhone ha una memoria sua, separata da Safari) lo incolla nel campo della scheda, link intero o solo codice. Il link `?code=<codice d'accesso>` apre la guida già dentro, senza cancello. Testi in `content.js → qr` e `ui.qr`. La prima scheda porta la **tabella oraria** dell'evento: una card info può avere un campo `orari` (giorni → righe `ora` + `cosa`) che viene disegnato come tabella invece che come prosa, ed entra anche nella ricerca, così "briefing" o "7:30" trovano la card.
- **Dormire**: mappa alloggi via iframe Stay22 (account aziendale reale `adventurelabsrl`, campagna `tgguida2026`). Riquadro alto **560 px** su telefono (600 da desktop): sotto una certa altezza Stay22 disegna mappa e hotel ma **non la traccia GPX**, senza nessun errore — misurato su iPhone, a 420 px la traccia non c'era, da 520 in su sì ([issue #23](https://github.com/advlabbik/tg-guida/issues/23), portato dal template il 19/9/2026). In locale la traccia non si vede comunque: il GPX lo scarica Stay22 dai suoi server, e `localhost` non lo raggiunge.
...
**Chi legge il QR al banco: il gestionale `bas-event-manager`, non `tg-checkin`.** Il QR di questa app (`TG26.<pettorale>.<firma>`) lo inquadra lo scanner del gestionale di check-in ([`advlabbik/bas-event-manager`](https://github.com/advlabbik/bas-event-manager), evento `tg-2026`), che lo riconosce anche dentro un link e apre la scheda della persona; la firma non la verifica, per scelta. I pettorali li legge da `event_bibs` a ogni apertura, quindi un iscritto nuovo si vede senza caricare file. L'app operatore `advlabbik/tg-checkin`, nata come pilota del QR ad agosto, **è in pensione e non si usa** (decisione del 19/9/2026): non va configurata né data agli operatori. Link e PIN del gestionale stanno sulla pagina Notion «App evento — link e codici d'accesso».

**Ramo `qr-nell-app` (17/9/2026) — il QR del ritiro pacco dentro l'app, e `?code=` nel cancello.** È l'eccezione decisa da Francesco alla regola «il motore non si tocca qui»: portato **dal template** (event-app-template#37), non da un'altra app. Due file copiati identici (`qr.js`, `lean-qr.js`), circa 120 righe ricopiate in `index.html`, testi IT/EN, stile, icona, service worker a `tg-guida-v45` con la pagina in cache senza `?code`/`?qr`. Checklist del piano fatta girare in locale su Chromium (nessun errore, QR disegnato e riletto uguale al codice). **Restano** il deploy di prova e le prove dal telefono, poi il merge **entro il 23/9** o non si fa per questa edizione. `start_url` nel manifest resta `./` finché non c'è il codice vero. Piano completo nel README della [PR #30](https://github.com/advlabbik/tg-guida/pull/30).
...
**23/9/2026 — #40 e #32 su `main` insieme** (ramo `golive-qr-23set`, cache `tg-guida-v48`): la Home senza la scadenza del 27 agosto e il QR del ritiro pacco dentro l'app. Deciso il 23/9: **il QR si vede solo nell'app**, l'email transazionale porta il link `?code=PIONEER26&qr={{ contact.TG_TOKEN }}` e il codice in chiaro da incollare nell'app installata, non l'immagine. Provato in locale in Chrome headless a 375 px (link senza cancello e barra pulita, scheda aperta col pettorale giusto, QR riletto con un decoder uguale al token, ricarica, link di un altro, offline, codice incollato intero/minuscolo/sbagliato, EN, ricerca, cache senza parametri, nessun errore JS). Restano le prove dal telefono della #32, da fare in produzione prima dell'invio.

**23/9/2026 sera — `whip-pagina-sito`** (cache `tg-guida-v50`): sotto la mappa WHIP del Live c'è il tasto «Pagina live sul sito» / «Live page on the website», che apre `https://www.trentinogravel.com/live` (in EN con `?lang=en`), la pagina nuova del sito (repo `advlabbik/trentinogravel`, commit `4c366e1`) con lo stesso iframe WHIP, da mandare a chi segue da casa. Campo `whipPagina` in `content.js`, IT e EN: **a evento finito vanno svuotati `whipUrl` e `whipPagina`**, e sul sito tolta o cambiata `/live`.

**23/9/2026 sera — `numero-e-whip`** (cache `tg-guida-v49`): la scheda del QR dice «Numero 2» / «Number 2» invece di «Pettorale» (non è una gara), e la sezione Live mostra la mappa pubblica WHIP `https://www.whip.live/event-tracking/TG26` ([#37](https://github.com/advlabbik/tg-guida/issues/37)), dal vademecum WHIP del 23/9. Il link di adesione con il PIN (`whip.live/in/event/TG26`) è andato ai partecipanti nell'email transazionale del 23/9, non nell'app. La card Info `whip` dal 26/9 (cache `tg-guida-v54`) dice di aprire il link WHIP dell'email del 23/9 e inserire il PIN, perché cercando l'evento dentro l'app WHIP non si trova; il PIN resta solo nell'email. Il 26/9 anche la domenica degli arrivi è passata dalle 14:00 alle 12:00 (cache `tg-guida-v53`).

**Ramo `avvisi-e-stay22` (19/9/2026, mergiato il 23/9).** Tolto l'avviso della Home e ogni riferimento alla scadenza del 27 agosto per il certificato (box «Da sapere adesso», voce 1 della checklist, scheda Info, IT e EN): era scaduta e la Home la mostrava ancora. `avvisi` resta vuoto, e vuoto il box non si disegna. Riquadro Stay22 a 560 px ([#23](https://github.com/advlabbik/tg-guida/issues/23)). Cache a `tg-guida-v47`: **al merge va più alta di quella su `main` in quel momento**, perché anche `qr-nell-app` e `meteo-previsione-percorso` dichiarano v47.
...
numeri ufficiali sono cambiati nel pomeriggio del 17/9, decisi da Andrea: **Corto 210×3.000 / Medio 353×6.740 / Lungo 371×7.400** (prima 216×3.000 / 357×7.100 / 376×7.900; lo scarto col GPX era diventato piu' largo di prima (2,7% sul Corto).
