...
**Schema condiviso multi-evento (24/8/2026, migrazione `eventi_condivisi`).** La convenzione iniziale prevedeva un set di tabelle prefissato per evento (`tg_*` per Trentino Gravel, `tt_*` per una futura Tuscany Trail...) nello stesso progetto — mai applicata ad altri eventi. È stata sostituita subito da uno schema unico condiviso: tabella `eventi` (id/slug/nome) + tabelle generiche `push_subscriptions`/`broadcast_messages`/`feedback`/`push_subscribe_attempts`/`feedback_attempts` con FK `evento_id`. `config.js` porta l'id fisso della riga `trentino-gravel` (`TG_EVENTO_ID`); la Edge Function `tg-send-broadcast` risolve il proprio evento dal secret `EVENTO_SLUG`. Ogni futura app-evento userà le stesse tabelle, filtrate dal proprio `evento_id` — niente nuovo set di tabelle né nuovo progetto Supabase per evento.
