...
**Ramo `live-manca-e-mappa-libera` (17/9/2026) — due cose dalla prova traccia di Andrea, portate dal template** (event-app-template, ramo `fix/live-manca-e-mappa-libera`): il dislivello che manca nella prima riga della Live, e la mappa della Live che si lascia trascinare senza tornare da sola alla posizione, con il tasto «Torna a me». Service worker a `tg-guida-v46` (la v45 è del ramo `qr-nell-app`: **chi unisce per secondo controlla che il numero sia più alto di quello in `main`**). Provato in locale con posizione simulata; **da provare in bici con il GPS vero** prima del merge, entro il 23/9. Gli altri due punti del feedback: il meteo aspetta di capire da Andrea cosa ha visto, il WHIP aspetta il link della mappa pubblica e si fa per ultimo.
