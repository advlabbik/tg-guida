...
- **Meteo nella scheda del percorso** (dal template, 17/9/2026, ramo `meteo-previsione-percorso`): sotto «Che tempo aspettarsi», da 14 giorni prima dell'evento, la previsione di Open-Meteo per i giorni dell'evento per ogni località della tabella, con la quota del posto passata al modello; sotto restano le temperature tipiche degli ultimi anni, che sono quelle da leggere prima.
...
**Ramo `meteo-previsione-percorso` (17/9/2026) — la previsione di Open-Meteo nella scheda del percorso, portata dal template** (event-app-template, ramo `feat/previsione-nella-vista-percorso`). È il punto 2 del feedback di Andrea dalla prova traccia. Sopra la tabella delle temperature tipiche compare la previsione per i giorni dell'evento delle stesse località, con la quota passata al modello; le coordinate delle undici località sono in `content.js → percorsi[].meteo[]`, agganciate alla traccia V3.0. Service worker a `tg-guida-v47` (v45 QR, v46 Live: **chi unisce controlla che il numero sia più alto di quello in `main`**). Provato in locale con una risposta finta di Open-Meteo; da guardare online con la previsione vera.
